...
const ProductInfo = ({ classes, productId, meta, structuredData }) => {
  ...
  const handleGetProductDetails = useCallback(async () => {
    ...
    const res = await dispatch(product.getGemStoneDetailsById(productId));
...
export const getServerSideProps = async (context) => {
  ...
  // get product details
  const productDetails = await axios.get(`${origin}/gemstone/${productId}`);
  ...
  */
  ...
  if (productData.success) {
    ...
    const { sgdPrice, variety,
      mediaUrl, shape, weight, primaryColour,
    } = mutated;
    ...
    const data = {
      ...
      price: 0,
      name: `${shape}, ${weight} Ct., ${primaryColour}, ${variety}`,
    };
