...
const ProductInfo = ({ classes, productId, meta, structuredData }) => {
  ...
  const handleGetProductDetails = useCallback(async () => {
    ...
    const res = await dispatch(product.getInhouseDetailsById(productId));
...
export const getServerSideProps = async (context) => {
  ...
  // get product details
  const productDetails = await axios.get(`${origin}/inhouse-diamond/${productId}`);
  ...
  */
  ...
  if (productData.success) {
    ...
    const { sgdPrice,
      diamondImage, shape, weight, color, clarity, cutGrade
    } = mutated;
    ...
    const data = {
      ...
      price: 0,
      name: `${shape}, ${weight} Ct., ${color}, ${clarity} ${cutGrade} Cut Diamond`,
    };


    const image = data.images.length > 0 ? data.images[0] : null; // data.variants[0]?.images[0] || websiteImage;
